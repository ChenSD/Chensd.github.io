...
1. 根据前人文献，确定所需要分析成分的标准电极点，标准时间窗。
2. 
...
that study a signal in both the Time and Frequency Domains (TFD) simultaneouslyTime-Frequency Analysis, TFA
2. Event-related spectral perturbations, ERSP
...
5. 确定好感兴趣的TFD之后, 就需要从感兴趣的TFD中提取每个实验条件的节律能量. 该指标为最后所使用的统计指标.

## General Steps

1. 总体步骤: eeglab里面前面的preprocessing是一样的 ICA是一样的 只是后面处理的时候选TFA或者ERP
