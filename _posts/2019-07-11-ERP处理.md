...
## 3. Evoked ERP-ERO Toolbox使用经验

该工具包只进行时域，频域或时频的组分析，并不提供预处理功能。因而可以很好地替代EEGLAB的Study功能。

1. 大体使用步骤：
    1. 首先需要将预处理后的数据汇总得到一个四维数据。该工具包内提供了汇总工具Evoked ERP ERO_v1.1_Forming_fourth_order_tensor_demo data。就是一个代码，运行出现GUI界面，填写所需参数，选择所需数据，就可以傻瓜导出后续分析所需的四维数据。
	2. 根据manual进行时域或时频分析。
