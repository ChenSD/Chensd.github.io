...
* 首先需要将预处理后的数据汇总得到一个四维数据。该工具包内提供了汇总工具Evoked ERP ERO_v1.1_Forming_fourth_order_tensor_demo data。就是一个代码，运行出现GUI界面，填写所需参数，选择所需数据，就可以傻瓜导出后续分析所需的四维数据。
* 汇总原理：平均试次维度，计算出所有被试，所有条件，所有时间点，所有通道的平均值。
最后的数据就是一个四维数据，在Matlab中为mat格式文件；时频分析的数据为五维数据，相比ERP汇总数据多了一个频率维度。
